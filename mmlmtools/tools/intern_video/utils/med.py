 ...
 * For full license text, see LICENSE.txt file in the repo root orhttps://opensource.org/licenses/BSD-3-Clause
 * By Junnan Li
    ...
    def forward(
        ...
        if is_cross_attention:
            if hidden_states.shape[0] != encoder_hidden_states.shape[0]:
                ...
                encoder_attention_mask = encoder_attention_mask[:hidden_states.
                                                                shape[0]]
...
class BertLMPredictionHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        # The output weights are the same as the input embeddings, but there
        # is an output-only bias for each token.
...
class BertModel(BertPreTrainedModel):
    ...
    def _prune_heads(self, heads_to_prune):
        ...
        heads_to_prune: dict of {layer_num: list of
    ...
    def get_extended_attention_mask(self, attention_mask: Tensor,
        ...
        Arguments:
            attention_mask (:obj:`torch.Tensor`): Mask with ones indicating
    ...
    def forward(
        ...
        encoder_hidden_states  (:obj:`torch.FloatTensor` of shape
            ...
            encoder. Used in the cross-attention if the model is configured as
            a decoder.
        encoder_attention_mask (:obj:`torch.FloatTensor` of shape
            :obj:`(batch_size, sequence_length)`, `optional`): Mask to avoid
            performing attention on the padding token indices of the encoder
            input. This mask is used in the cross-attention if the model is
        ...
        past_key_values (:obj:`tuple(tuple(torch.FloatTensor))` of length
            ...
            states of the attention blocks. Can be used to speed up decoding.
            If :obj:`past_key_values` are used, the user can optionally input
            only the last :obj:`decoder_input_ids` (those that don't have their
            past key value states given to this model) of shape
            :obj:`(batch_size, 1)` instead of all :obj:`decoder_input_ids` of
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_attention_mask (:obj:`torch.FloatTensor` of shape
            ...
            of the encoder input. This mask is used in the cross-attention if
            the model is configured as a decoder. Mask values selected in
            ``[0, 1]``:
                - 1 for tokens that are **not masked**,
                - 0 for tokens that are **masked**.
        labels (:obj:`torch.LongTensor` of shape
            :obj:`(batch_size, sequence_length)`, `optional`): Labels for
            computing the left-to-right language modeling loss (next word
            prediction). Indices should be in
            ...
            with labels n ``[0, ..., config.vocab_size]``
        past_key_values (:obj:`tuple(tuple(torch.FloatTensor))` of length
            :obj:`config.n_layers` with each tuple having 4 tensors of shape
            :obj:`(batch_size, num_heads, sequence_length - 1,
            embed_size_per_head)`): Contains precomputed key and value hidden
            states of the attention blocks. Can be used to speed up decoding.
            If :obj:`past_key_values` are used, the user can optionally input
            only the last :obj:`decoder_input_ids` (those that don't have
            their past key value states given to this model) of shape
            :obj:`(batch_size, 1)` instead of all :obj:`decoder_input_ids` of
            shape :obj:`(batch_size, sequence_length)`.
        use_cache (:obj:`bool`, `optional`):
            If set to :obj:`True`, :obj:`past_key_values` key value states are
