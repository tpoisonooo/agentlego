...
from mmengine.utils import is_main_process
...
class BertForImageCaptioning(BertPreTrainedModel):
    ...
    def encode_forward(self,
        ...
        if is_training:
            ...
            sequence_output_masked = sequence_output[
                masked_pos ==
            ...
            if masked_pos_img is not None:
                ...
                # num_masks_in_batch * hidden_size
                sequence_output_masked_img = sequence_output_img[
                    masked_pos_img == 1, :]
                # img cclin
                img_feat = self.cls_img_feat(sequence_output_masked_img)
                masked_loss_img = self.loss_img_feat(img_feat.float(),
                                                     masked_token_img)
    ...
    def prod_generate(
        ...
        if add_od_labels:
            od_labels_posids = torch.arange(
                od_labels_start_posid,
                od_labels_start_posid + od_labels_len,
                dtype=torch.long,
                device=device)
            position_ids = torch.cat([position_ids, od_labels_posids])
        ...
        while True:
            ...
            input_ids = torch.cat([last_token, mask_ids], dim=1)
            position_ids = torch.arange(
                cur_len - 1, cur_len + 1, dtype=torch.long, device=device)
            attention_mask = torch.ones(
    ...
    def prod_no_hidden_generate(
        ...
        def _prepare_inputs(cur_ids):
            ...
            token_len = cur_len + 1
            position_ids = torch.arange(
                token_len, dtype=torch.long, device=device)
            if add_od_labels:
                od_labels_posids = torch.arange(
                    od_labels_start_posid,
                    od_labels_start_posid + od_labels_len,
