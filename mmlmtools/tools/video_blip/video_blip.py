...
# https://github.com/yukw777/VideoBLIP/blob/main/video_blip/model.py
# by Yu Keunwoo Peter under the MIT license

import torch
import torch.nn as nn
from transformers import (AutoModelForCausalLM, AutoModelForSeq2SeqLM,
                          BatchEncoding, Blip2Config,
...
def process(
    ...
    if video is not None:
        ...
        inputs['pixel_values'] = inputs.pixel_values.view(
            batch, time, channel, height, weight).permute(0, 2, 1, 3, 4)
    return inputs
...
class VideoBlipVisionModel(Blip2VisionModel):
    ...
    def forward(
        ...
        :param pixel_values: a tensor of shape (batch, channel, t, h, w)

        :returns:
            last_hidden_state: a tensor of shape (batch, t * seq_len,
                hidden_size)
            pooler_output: a tensor of shape (batch, t, hidden_size)
            hidden_states:
                a tuple of tensors of shape (batch, t * seq_len, hidden_size),
                one for the output of the embeddings + one for each layer
            attentions:
                a tuple of tensors of shape (batch, t, num_heads, seq_len,
                seq_len), one for each layer
        """
        ...
        # flatten along the batch and time dimension to create a tensor of
        ...
        hidden_states = (
            ...
            if vision_outputs.hidden_states is not None else None)
